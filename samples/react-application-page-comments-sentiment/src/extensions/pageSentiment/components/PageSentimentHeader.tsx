...
export default class PageSentimentHeader extends React.Component<
  IPageSentimentHeaderProps,
...
> {
  ...
  public componentWillMount(): void {
    ...
    this._listId =
      this.props.context.pageContext.list &&
      this.props.context.pageContext.list.id.toString();
    this._listItemId =
      this.props.context.pageContext.listItem &&
      this.props.context.pageContext.listItem.id.toString();
  ...
  private async _getTop3PageComments(): Promise<IPageComment[]> {
    const pageCommentsEndpoint: string = `${this._baseUrl}/_api/web/lists('${this._listId}')/GetItemById(${this._listItemId})/Comments?$top=3&$inlineCount=AllPages`;

    const response: SPHttpClientResponse = await this._spHttpClient.get(
      pageCommentsEndpoint,
  ...
  private async _detectLanguage(comment: IPageComment): Promise<string> {
    const httpOptions: IHttpClientOptions = this._prepareHttpOptionsForTextApi(
      [comment],
      null
    );
    const cognitiveResponse: HttpClientResponse = await this._httpClient.post(
    ...
    return cognitiveResponseJSON.documents[0].detectedLanguages[0].iso6391Name;
  }

  private async _getSentimentFromPageComments(
    comments: IPageComment[]
  ): Promise<number> {
    const detectedLanguage: string = await this._detectLanguage(comments[0]); //for demo just taking the first comment to detect language, assuming all comments in the same language
    const httpOptions: IHttpClientOptions = this._prepareHttpOptionsForTextApi(
      comments,
      detectedLanguage
    );
    ...
    const scoreSum: any = cognitiveResponseJSON.documents.reduce(
      (total, item) => {
  ...
  public render(): React.ReactElement<IPageSentimentHeaderProps> {
    ...
    return (
      <div className={styles.sentimentHeader}>
        <Rating size={RatingSize.Large} rating={rating} readOnly={true} />
      </div>
    );
  }

  private _prepareHttpOptionsForTextApi(
    comments: IPageComment[],
    language: string
  ): IHttpClientOptions {
    const body: any = {
