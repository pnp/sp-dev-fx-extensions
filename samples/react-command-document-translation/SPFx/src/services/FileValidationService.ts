...
import { DocumentInfo, SUPPORTED_FILE_EXTENSIONS, MAX_FILE_SIZE } from '../models/TranslationModels';
...
export class FileValidationService {
  ...
  private static validateSingleItem(item: RowAccessor): DocumentInfo {
    ...
    // Validate file size (Azure Document Translation limit is 40MB)
    if (size > MAX_FILE_SIZE) {
      documentInfo.isSupported = false;
      documentInfo.errorMessage = `File size (${this.formatFileSize(size)}) exceeds maximum limit of ${this.formatFileSize(MAX_FILE_SIZE)}`;
      return documentInfo;
    }
  ...
  public static getValidationSummary(documents: DocumentInfo[]): string {
    ...
    if (unsupported === 0) {
      return `${supported} file${supported !== 1 ? 's' : ''} ready for translation (${totalSize})`;
    } else {
      return `${supported} file${supported !== 1 ? 's' : ''} ready for translation, ${unsupported} file${unsupported !== 1 ? 's' : ''} skipped (${totalSize})`;
    }
  }
}
