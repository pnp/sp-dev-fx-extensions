...
export class ConfigurationService {
  ...
  private cachedConfig: IChatbotConfiguration | null = null;
  ...
  public async getConfiguration(): Promise<IChatbotConfiguration> {
    try {
  ...
  private async getConfigFromListWithRetry(): Promise<IChatbotConfiguration | null> {
  ...
  private async getConfigFromList(): Promise<IChatbotConfiguration | null> {
    try {
  ...
  private async getConfigFromTenantExtensionWithRetry(): Promise<IChatbotConfiguration | null> {
    const retryAttempts = this.cachedConfig?.errorRetryAttempts || ConfigurationService.DEFAULT_RETRY_ATTEMPTS;
  ...
  private mapListItemToConfig(fields: any): IChatbotConfiguration {
    return {
  ...
  private validateConfig(config: any): IChatbotConfiguration {
    if (!config.botURL || !config.clientID || !config.authority || !config.customScope) {
      throw new Error('Invalid configuration: missing required fields');
    }
    return config as IChatbotConfiguration;
  }
  ...
  private getFromCache(): IChatbotConfiguration | null {
  ...
  private updateCache(config: IChatbotConfiguration): void {
    this.cachedConfig = config;
