...
export const filterItems = (items: IListViewItem[], searchText: string | undefined | null): IListViewItem[] => {
  ...
  return items.filter((item) => {
    return (
      (item.Name ?? '').toLowerCase().includes(cleanSearchText) ||
      (item.Type ?? '').toLowerCase().includes(cleanSearchText) ||
      (item.Items?.toString() ?? '').includes(cleanSearchText) ||
      (item.Modified ?? '').toLowerCase().includes(cleanSearchText) ||
      (item.Description ?? '').toLowerCase().includes(cleanSearchText)
    );
  });
};
